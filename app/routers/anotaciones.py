...
def get_anotaciones(equipo_id: int, db: sqlite3.Connection = Depends(get_db), current_user: dict = Depends(get_current_user)):
    try:
        cursor = db.cursor()
        cursor.execute("SELECT image_id, annotations, comentario FROM anotaciones_imagenes WHERE equipo_id = ?", (equipo_id,))
        ...
        for row in rows:
            ...
            comentarios_map[image_id] = row["comentario"] or ""
                
        return {
            "equipo_id": equipo_id,
            "anotaciones": anotaciones_map,
            "comentarios": comentarios_map
        }
...
def save_anotaciones(data: AnotacionSaveRequest, db: sqlite3.Connection = Depends(get_db), current_user: dict = Depends(get_current_user)):
    try:
        ...
        """, (data.equipo_id, data.image_id, annotations_json, data.comentario))
        db.commit()
        return {
            "status": "success",
