...
export function HistorialActivoModal({ equipoId, codigoActivo, nombreActivo, isOpen, onClose }) {
      ...
      default:
        return (
          <span className="bg-slate-800 text-slate-300 font-black px-2.5 py-1 rounded-lg text-xs flex items-center gap-1 border border-slate-600">
            <Slash className="w-3.5 h-3.5" /> FUERA RUTA
          </span>
        );
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md p-4 flex items-center justify-center animate-in fade-in duration-150">
      <div className="bg-slate-900 border-3 border-sky-500 max-w-md w-full max-h-[85vh] rounded-3xl p-5 flex flex-col justify-between shadow-2xl space-y-4" style={{ backgroundColor: '#0f172a', borderColor: '#0284c7' }}>
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b-2 border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <History className="w-6 h-6 text-sky-400" style={{ color: '#38bdf8' }} />
            <div>
              <span className="font-mono font-black text-sky-400 text-xs block" style={{ color: '#38bdf8' }}>{codigoActivo}</span>
              <h3 className="font-black text-xl text-white m-0 leading-tight" style={{ color: '#ffffff' }}>Historial del Activo</h3>
            </div>
          </div>
          <button
            type="button"
        ...
        {/* Modal Content - Lista de Inspecciones Anteriores */}
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {loading && historial.length === 0 ? (
            ...
            historial.map((item, index) => (
              <div
                key={item.id || index}
                className="bg-slate-950 border-2 border-slate-800 p-4 rounded-2xl space-y-2"
              ...
              >
                ...
                {item.observaciones && (
                  <p className="text-sm font-semibold text-slate-200 bg-slate-900 p-2.5 rounded-xl border border-slate-800 m-0" style={{ color: '#e2e8f0', backgroundColor: '#0f172a' }}>
                    "{item.observaciones}"
                  </p>
                )}

                {item.hallazgos && (
                  <div className="bg-sky-950/60 p-2.5 rounded-xl border border-sky-800 text-xs font-semibold" style={{ backgroundColor: '#0c4a6e', borderColor: '#0369a1', color: '#bae6fd' }}>
                    <span className="font-black text-sky-300 block mb-0.5" style={{ color: '#7dd3fc' }}>🤖 Diagnóstico Reciente (Gemini):</span>
                    {item.hallazgos}
                  </div>
                )}
              </div>
